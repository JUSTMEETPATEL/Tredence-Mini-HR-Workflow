...
export function TaskNode({ data, selected }: { data: Record<string, unknown>; selected?: boolean }) {
  return (
    <div className={`bg-white border-2 rounded-xl shadow-[var(--shadow-node)] min-w-[220px] transition-all duration-200 ${selected ? 'border-[var(--node-task)] ring-2 ring-[var(--node-task)]/30' : 'border-[var(--node-task)]/40'}`}>
