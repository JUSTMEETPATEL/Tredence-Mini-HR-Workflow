...
export function AutomatedStepNode({ id, data, selected }: { id: string; data: Record<string, unknown>; selected?: boolean }) {
  return (
    <div className={`relative bg-white border-2 rounded-xl shadow-[var(--shadow-node)] min-w-[220px] transition-all duration-200 ${selected ? 'border-[var(--node-automated)] ring-2 ring-[var(--node-automated)]/30' : 'border-[var(--node-automated)]/40'}`}>
