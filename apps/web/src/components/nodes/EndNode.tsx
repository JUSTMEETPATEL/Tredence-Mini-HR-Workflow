...
export function EndNode({ id, data, selected }: { id: string; data: Record<string, unknown>; selected?: boolean }) {
  return (
    <div className={`relative bg-white border-2 rounded-xl shadow-[var(--shadow-node)] min-w-[200px] transition-all duration-200 ${selected ? 'border-[var(--node-end)] ring-2 ring-[var(--node-end)]/30' : 'border-[var(--node-end)]/40'}`}>
