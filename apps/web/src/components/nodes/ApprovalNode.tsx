...
export function ApprovalNode({ data, selected }: { data: Record<string, unknown>; selected?: boolean }) {
  return (
    <div className={`bg-white border-2 rounded-xl shadow-[var(--shadow-node)] min-w-[220px] transition-all duration-200 ${selected ? 'border-[var(--node-approval)] ring-2 ring-[var(--node-approval)]/30' : 'border-[var(--node-approval)]/40'}`}>
